...
│   └── 運用マニュアル.md
├── README.md
└── 本番環境セットアップ手順.md … 本番ホスティングへ移す際の手順
```

本番ホスティングへの移行手順は `本番環境セットアップ手順.md` を参照してください。
...
社内の他のPCからも動作確認したい場合は、GitHub Codespacesを使う方法がおすすめです（ファイアウォール設定が不要なため）。手順はチームメンバーにお尋ねください。
